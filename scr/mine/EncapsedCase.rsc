...
public void encapsedCase(list[Expr] expressions,map[str,str] moles,str tempName){
		Expr mixed;
		for(mixed <- expressions){ 
			switch (mixed) {
				case scalar(string(X)) : {
					appendToFile(|file://C:/xampp/htdocs/smarty/templates/<tempName>.tpl|, " <X> ");
				}
				...
				default : println ("case not supported");
			}
		}
}